@echo off
setlocal

REM ============================================================
REM PassKinko Release Build Script
REM Version: V002.000.000
REM Output : RELEASE\PassKinko_V002.000.000
REM ============================================================

cd /d "%~dp0"

set "APP_NAME=PassKinko"
set "VERSION=V002.000.000"

set "PROJECT_DIR=%~dp0PassKinko.App"
set "PROJECT_FILE=%PROJECT_DIR%\PassKinko.App.csproj"

set "WORK_DIR=%TEMP%\PassKinko_BUILD_WORK_%VERSION%_%RANDOM%%RANDOM%"
set "PUBLISH_DIR=%WORK_DIR%\publish_tmp"
set "OBFUSCAR_CONFIG=%WORK_DIR%\obfuscar.xml"
set "RELEASE_ROOT=%~dp0RELEASE"
set "RELEASE_DIR=%RELEASE_ROOT%\PassKinko_%VERSION%"
set "ZIP_FILE=%~dp0PassKinko_%VERSION%_Windows.zip"

echo [INFO] PassKinko Release Build
echo [INFO] Version: %VERSION%
echo.

echo [1/5] Cleaning work folder...
if exist "%WORK_DIR%" (
    rmdir /s /q "%WORK_DIR%"
    if errorlevel 1 (
        echo [ERROR] Failed to remove BUILD_WORK folder.
        echo [ERROR] Close Explorer or running app and try again.
        if not "%PASSKINKO_NO_PAUSE%"=="1" pause
        exit /b 1
    )
)

echo.
echo [2/5] Restoring project...
dotnet restore "%PROJECT_FILE%"
if errorlevel 1 (
    echo [ERROR] dotnet restore failed.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

echo.
echo [3/5] Publishing Release...
dotnet publish "%PROJECT_FILE%" -c Release -o "%PUBLISH_DIR%" --self-contained false /p:PublishSingleFile=false
if errorlevel 1 (
    echo [ERROR] dotnet publish failed.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

echo.
echo [3.5/5] Obfuscating...
if exist "%WORK_DIR%\obfuscated" rmdir /s /q "%WORK_DIR%\obfuscated"
copy /y "%PUBLISH_DIR%\パス金庫.dll" "%PUBLISH_DIR%\PassKinko.dll" >nul
if errorlevel 1 (
    echo [ERROR] Failed to prepare DLL for Obfuscar.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)
(
    echo ^<?xml version='1.0'?^>
    echo ^<Obfuscator^>
    echo   ^<Var name="InPath" value="%PUBLISH_DIR%" /^>
    echo   ^<Var name="OutPath" value="%WORK_DIR%\obfuscated" /^>
    echo   ^<Var name="KeepPublicApi" value="false" /^>
    echo   ^<Var name="HidePrivateApi" value="true" /^>
    echo   ^<Var name="RenameProperties" value="false" /^>
    echo   ^<Var name="RenameEvents" value="true" /^>
    echo   ^<Var name="RenameFields" value="true" /^>
    echo   ^<Module name="PassKinko.dll" /^>
    echo ^</Obfuscator^>
) > "%OBFUSCAR_CONFIG%"
"%USERPROFILE%\.nuget\packages\obfuscar\2.2.38\tools\Obfuscar.Console.exe" -s "%OBFUSCAR_CONFIG%"
set "OBFUSCAR_EXIT=%ERRORLEVEL%"
if not "%OBFUSCAR_EXIT%"=="0" (
    echo [WARN] Obfuscar failed. Continuing with the published DLL.
    del /q "%PUBLISH_DIR%\PassKinko.dll" >nul 2>&1
    goto AfterObfuscation
)
if not exist "%WORK_DIR%\obfuscated\PassKinko.dll" (
    echo [ERROR] Obfuscated DLL was not created.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)
copy /y "%WORK_DIR%\obfuscated\PassKinko.dll" "%PUBLISH_DIR%\パス金庫.dll"
if errorlevel 1 (
    echo [ERROR] Failed to copy obfuscated DLL.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)
del /q "%PUBLISH_DIR%\PassKinko.dll"

:AfterObfuscation

echo.
echo [4/6] Checking publish output...

dir /b "%PUBLISH_DIR%\*.exe" >nul 2>&1
if errorlevel 1 (
    echo [ERROR] Release EXE was not created.
    dir "%PUBLISH_DIR%"
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

dir /b "%PUBLISH_DIR%\*.dll" >nul 2>&1
if errorlevel 1 (
    echo [ERROR] Release DLL was not created.
    dir "%PUBLISH_DIR%"
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

dir /b "%PUBLISH_DIR%\*.runtimeconfig.json" >nul 2>&1
if errorlevel 1 (
    echo [ERROR] runtimeconfig.json was not created.
    dir "%PUBLISH_DIR%"
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

dir /b "%PUBLISH_DIR%\*.deps.json" >nul 2>&1
if errorlevel 1 (
    echo [ERROR] deps.json was not created.
    dir "%PUBLISH_DIR%"
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

echo.
echo [5/6] Creating release folder...
if not exist "%RELEASE_ROOT%" mkdir "%RELEASE_ROOT%"

if exist "%RELEASE_DIR%" (
    rmdir /s /q "%RELEASE_DIR%"
    if errorlevel 1 (
        echo [ERROR] Failed to remove old release folder.
        if not "%PASSKINKO_NO_PAUSE%"=="1" pause
        exit /b 1
    )
)

mkdir "%RELEASE_DIR%"
xcopy "%PUBLISH_DIR%\*" "%RELEASE_DIR%\" /E /I /Y >nul
if errorlevel 1 (
    echo [ERROR] Failed to copy publish files to release folder.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

if exist "%RELEASE_DIR%\*.pdb" del /q "%RELEASE_DIR%\*.pdb"

dir /b "%RELEASE_DIR%\*.exe" >nul 2>&1
if errorlevel 1 (
    echo [ERROR] Final EXE not found after copy.
    dir "%RELEASE_DIR%"
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

echo.
echo [6/6] Creating release ZIP...
if exist "%ZIP_FILE%" del /q "%ZIP_FILE%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "Compress-Archive -Path '%RELEASE_DIR%\*' -DestinationPath '%ZIP_FILE%' -Force"
if errorlevel 1 (
    echo [ERROR] Failed to create release ZIP.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

if not exist "%ZIP_FILE%" (
    echo [ERROR] Release ZIP was not created.
    if not "%PASSKINKO_NO_PAUSE%"=="1" pause
    exit /b 1
)

echo.
echo [OK] Release build completed.
echo.
echo Output:
echo %RELEASE_DIR%
echo.
echo Release ZIP:
echo %ZIP_FILE%
echo.
echo Main EXE:
dir /b "%RELEASE_DIR%\*.exe"
echo.
if not "%PASSKINKO_NO_PAUSE%"=="1" pause
exit /b 0



